        ...
        if(stat) {
            ...
            if(sticker.images && sticker.images.animRegular && sticker.images.animRegular.XXHDPI 
                && sticker.images.animRegular.XXHDPI.gcsPath) {
                    var imageUrl = config.get('gcs.urlPrefix') +  "/" + sticker.images.animRegular.XXHDPI.gcsPath;
...
function uploadOnCloudinary(originalImageUrl, sticker, callback) {
    var options = {
        folder: config.get('imageFolder.cloudinaryOrgImgFolder') + '/animRegular/' + sticker.stickerId,
        public_id: "original"
    }
...
function getImageMetaForAllResolution(uploadResult, callback) {
    ...
    for (key in resFactor.sticker) {
        ...
        animMeta.imageUrl = getTranformedUrl(uploadResult, animMeta, "gif")
        imageMeta.animRegular.push(animMeta)
        var staticMeta = {
            "resType" : key,
            "width": Math.round(base_width*resFactor.sticker[key]),
            "height": Math.round(base_height*resFactor.sticker[key])
        }
        staticMeta.imageUrl = getTranformedUrl(uploadResult, staticMeta, "png")
        imageMeta.staticRegular.push(staticMeta)
        ...
        miniMeta.imageUrl = getTranformedUrl(uploadResult, miniMeta, "png")
        imageMeta.mini.push(miniMeta)
    }
