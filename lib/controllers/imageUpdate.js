...
var regular_res_factor = {'HDPI':0.4888888889, 'LDPI':0.2222222222, 'MDPI':0.2888888889, 'XHDPI':0.6537037037, 'XXHDPI':1}
...
function getImagePaths(lcids, callback) { 
    var fileData = []
    var regular_image_folder = path.join(constants.dropbox_path, "2_Stickers", "1_Source files","Or_SticPNG")
    async.waterfall([
        (callback) => {
            if(lcids) {
                callback(null, lcids)
                ...
                readDir(regular_image_folder, (error, catIds) => {
                    if(error) {
                        callback(error, null)
                    }
                    else {
        ...
        (catIds, callback) => {
            async.each(catIds, (catId, callback) => {
                var folderPath = path.join(regular_image_folder, catId)
...
function uploadOnCloudinary(info, callback) {
    ...
    var options = {
        folder: "staging-regular/images/"+sticker.stickerId,
        public_id: "mini",
        effect: "trim"
    }
...
function getImageMetaForAllResolution(data, callback) {
    ...
    getImageDimension(dict, (error, values) => {
        if(error) {
            ...
            for (key in regular_res_factor) {
                var meta = {
                    "resType" : key,
                    "width": Math.round(base_width*regular_res_factor[key]),
                    "height": Math.round(base_height*regular_res_factor[key])
                };
...
function updateStickerDbForMiniImageUpload(info, callback) {
    ...
    async.parallel({
        updateLegacySticker: (callback) => {
            LegacySticker.update({sId: sticker.lsid, catId: sticker.lcid}, {$set: {'img_avail.st': true}}, (error, response) => {
                if(error) {
                    callback(error, null)
                }
                else {
        ...
        updateSticker: (callback) => {
            Sticker.update({stickerId: sticker.stickerId}, {$set: {'images.staticRegular': sticker.images.staticRegular}}, (error, response) => {
                if(error) {
                    callback(error, null)
                }
                else {
            ...
            if(error) {
                ...
                async.parallel({
                    legacyStickerUpload: (callback) => {
                        ...
                        var filepath = "stk_"+image.resType.toUpperCase()+'_'+sticker.lcid+'/'+sticker.lsid
                    ...
                    stickerUpload: (callback) => {
                        ...
                        var filepath = "stickers/"+sticker.stickerId+"/staticRegular/"+image.resType.toUpperCase()+'.png'
                        var file = bucket.file(filepath);
                        ...
                        file.save(imageBuffer, {
                            ...
                            if (error) {
                                ...
                                sticker.images.staticRegular[image.resType]["gcsPath"] = filepath
                                sticker.images.staticRegular[image.resType]["width"] = image.width
                                sticker.images.staticRegular[image.resType]["height"] = image.height
                                sticker.images.staticRegular[image.resType]["size"] = imageData.size
                                callback(null, filepath)
                            }
                        });
                    }
...
function runScript(catIds) {
    getImagePaths(catIds, (error, fileData) => {
        if(error) {
            ...
            console.log("Total Image Paths Found is : "+ fileData.length)
            var current_count = 0;
            async.eachSeries(fileData, (data, callback) => {
                current_count += 1;
                console.log("Processing Image for catId : "+data.catId + " and sId : "+ data.sId + " and count is :"+current_count)
                async.waterfall([
                    (callback) => {
