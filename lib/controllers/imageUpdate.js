...
function getStickerId(info, callback) {
...
function uploadOnCloudinary(imageData, callback) {
    var options = {
        folder: "testing/images/"+imageData.stickerId,
        public_id: "mini",
        effect: "trim"
    }
    cloudinary.v2.uploader.upload(imageData.path, options, (error, result) => {
        if(error) {
            callback(error, null)
        }
        else {
...
function updateStickerDbForMiniImageUpload(info, callback) {
    ...
    async.parallel({
        updateLegacySticker: (callback) => {
            callback(null, 'done')
            // LegacySticker.update({sId: sticker.sId, catId: sticker.catId}, {$set: {'img_avail.stmn': true}}, (error, response) => {
            //     if(error) {
            //         callback(error, null)
            //     }
            //     else {
            ...
            if(error) {
                ...
                async.parallel({
                    legacyStickerUpload: (callback) => {
                        ...
                        var filepath = "stk_mini_"+image.resType.toUpperCase()+'_'+sticker.catId+'/'+sticker.sId
                        var file = bucket.file(filepath);
                        var legacyImageData = imageData.toString("base64")
    ...
    getImagePaths(catIds, (error, fileData) => {
        if(error) {
            ...
            async.eachSeries(fileData, (data, callback) => {
                ...
                async.waterfall([
                    (callback) => {
                        getStickerId(data, (error, value) => {
                            if(error) {
                                callback(error, null)
                            }
                            else {
                                data.stickerId = value.stickerId
                                callback(null, data)
                            }
                        })
                    },
                    (dataWithStickerId, callback) => {
                        uploadOnCloudinary(dataWithStickerId, (error, cloudinaryResponse) => {
                            if(error) {
                                callback(error, null)
                            }
                            else {
                    ...
                    (cloudinaryResponse, callback) => {
                        getImageMetaForAllResolution(cloudinaryResponse, (error, imagesResMeta) => {
                            if(error) {
                                callback(error, null)
                            }
                            else {
                                callback(null, imagesResMeta)
                            }
                        })
                    },
                    (imagesResMeta, callback) => {
                        var uploadInfo = {
                            sticker: data,
