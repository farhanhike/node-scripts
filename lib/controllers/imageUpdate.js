...
function getStickerWithoutMini() {
    LegacySticker.find({"img_avail.st": true, "img_avail.stmn": false}, {catId:1, sId:1}, function(error, legacySticker) {
        if(error) {
            console.log(error)
        }
        else {
            jsonfile.writeFile("sticker_without_mini.json", legacySticker, {spaces: 2}, function (err) {
