...
function checkMiniImageExistsInDropbox(legacyStickers, callback) {
    var finalResults = [];
    async.eachSeries(legacyStickers, function(legacySticker, callback) {
        var mini_image_folder = path.join(constants.dropbox_path, "2_Stickers", "1_Source files","Or_SticMINI")
        var miniStickerPath = path.join(mini_image_folder, legacySticker.catId, legacySticker.sId);
        fs.stat(miniStickerPath, (error, stat) => {
            if(error) {
                console.log("Error for filepath : "+miniStickerPath +' is ')
                callback()
            }
            else if (stat && stat.isFile()) {
                legacySticker.path = miniStickerPath
                finalResults.push(legacySticker)
                callback()
            }
            else {
                console.log("Cannot find filepath : "+miniStickerPath)
                callback()
            }
        })
    }, function(error) {
...
function uploadOnCloudinary(info, callback) {
    ...
    var options = {
        folder: "mini-from-gcs/images/"+sticker.stickerId,
        public_id: "mini",
        effect: "trim"
    }
...
function getStickerWithoutMini(callback) {
    LegacySticker.find({}, {catId:1, sId:1, img_avail:1}, function(error, legacyStickers) {
        if(error) {
            console.log(error)
            callback(error, null)
        }
            ...
            for(var i=0; i<legacyStickers.length; i++) {
                ...
                if(legacySticker.img_avail && legacySticker.img_avail.st == true && legacySticker.img_avail.stmn == false) {
                    var data = {
                        catId: legacySticker.catId,
                        sId: legacySticker.sId
                    }
                    result.push(data)
                }
            }
...
function runScript() {
    var imageUriPrefix = "https://sticker-images-contentqa.im.hike.in/" // staging
    // var imageUriPrefix = "https://sticker-images.im.hike.in/" // prod
    getStickerWithoutMini(function(error, legacyStickers) {
        if(error) {
            console.log(error)
        }
        else {
            console.log("Total Data : "+ legacyStickers.length)
            var currentCount = 1;
            async.eachLimit(legacyStickers, 1, (data, callback) => {
                console.log("Processing Image for catId : "+data.catId + " and sId : "+ data.sId + " current count : "+ currentCount)
                async.waterfall([
                    (callback) => {
                        getSticker(data, (error, value) => {
                            if(error) {
                                ...
                                if(value.images && value.images.staticRegular && value.images.staticRegular.XXHDPI && value.images.staticRegular.XXHDPI.gcsPath) {
                                    data.path = imageUriPrefix + value.images.staticRegular.XXHDPI.gcsPath
                                    callback(null, data, value)
                                }
                                else {
                                    callback("No XXHDPi Image path on gcs", null)
                                }
                            }
                        })
                    },
                    ...
                    if(error) {
                        console.log("Error for catId: "+data.catId + " sid: "+ data.sId + " with error", error);
                    }
                    else {
                        console.log("Update finsihed for catId: "+result.sticker.lcid + " sid: "+result.sticker.lsid + " stickerId : "+  result.sticker.stickerId);
                    }
                    currentCount += 1;
                    callback()
                })
