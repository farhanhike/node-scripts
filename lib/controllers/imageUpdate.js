...
function uploadOnCloudinary(info, callback) {
    ...
    var options = {
        folder: "staging-test/images/"+sticker.stickerId,
        public_id: "mini",
        effect: "trim"
    }
    cloudinary.v2.uploader.upload(fileMeta.imagePath, options, (error, result) => {
        if(error) {
            callback(error, null)
        }
        else {
...
function getAllStickers(callback) {
    Sticker.find({}).limit(10).exec(function(error, stickers) {
        if(error) {
            callback(error, null)
        }
        else {
...
function getDropboxImagePath(sticker, callback) {
    var regular_image_folder = path.join(constants.dropbox_path, "2_Stickers", "1_Source files","Or_SticPNG")
    var regularStickerPath = path.join(regular_image_folder, sticker.lcid, sticker.lsid);
    fs.stat(regularStickerPath, (error, stat) => {
        if(error) {
            console.log("Error for filepath : "+miniStickerPath +' is ')
            callback(error, null)
        }
        else if (stat && stat.isFile()) {
            callback(null, regularStickerPath)
        }
        else {
...
function getDropboxImagePath(sticker, callback) {
    ...
    fs.stat(regularStickerPath, (error, stat) => {
        if(error) {
            console.log("Error for filepath : "+regularStickerPath)
            callback(error, null)
        }
        else if (stat && stat.isFile()) {
            callback(null, regularStickerPath)
            ...
            callback("couldnt find path", null)
        }
    })
}
...
function runScript() {
    ...
    getAllStickers(function(error, stickers) {
        if(error) {
            console.log(error)
        }
        else {
            async.eachLimit(stickers, 1, (sticker, callback) => {
                if(sticker.stickerId && sticker.lcid && sticker.lsid) {
                    async.waterfall([
                        (callback) => {
                            getDropboxImagePath(sticker, (error, imageUrl) => {
                                var fileData = {
                                    lcid: sticker.lcid,
                                    lsid: sticker.lsid,
                                }
                                if(imageUrl) {
                                    fileData.imagePath = imageUrl
                                    callback(null, fileData, sticker)
                                }
                                else if(sticker.images && sticker.images.staticRegular && sticker.images.staticRegular.XXHDPI && sticker.images.staticRegular.XXHDPI.gcsPath) {
                                    fileData.imagePath = imageUriPrefix + sticker.images.staticRegular.XXHDPI.gcsPath;
                                    callback(null, fileData, sticker)
                                }
                                else {
                                    callback("No image url avialble", null)
                                }
                            })
                        },
                        ...
                        if(error) {
                            ...
                            console.log("Update finsihed for lcid: "+result.sticker.lcid + " sid: "+result.sticker.lsid + " stickerId : "+  result.sticker.stickerId);
                        }
                        callback()
                    })
                }
