...
function uploadOnCloudinary(info, callback) {
    ...
    var options = {
        folder: "missing-mini-stickers/images/"+sticker.stickerId,
        public_id: "mini",
        effect: "trim"
    }
...
function getStickerWithoutMini(callback) {
    LegacySticker.find({}, {catId:1, sId:1, img_avail:1}, function(error, legacyStickers) {
        if(error) {
            ...
            console.log("Total Stickers found :" + result.length)
            callback(null, result)
        }
    })
}
...
function runScript() {
    getStickerWithoutMini(function(error, legacyStickers) {
        if(error) {
            ...
            console.log(legacyStickers[0])
            checkMiniImageExistsInDropbox(legacyStickers, (error, fileData) => {
                if(error) {
                    console.log(error)
                }
                else {
                    var currentCount = 1;
                    async.eachLimit(fileData, 10, (data, callback) => {
