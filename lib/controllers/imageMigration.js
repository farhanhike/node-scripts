    ...
    getLegacyIds((error, legacyIds) => {
        if(error) {
            ...
            async.eachSeries(legacyIds, (legacyId, callback) => {
                console.log("Processing lcid : "+ sticker.lcid + " and lsid : "+ sticker.lsid + " current count : "+currentCount)
                currentCount += 1;
                Sticker.findOne({lcid: legacyId.lcid, lsid: legacyId.lsid}, function(error, sticker) {
                    if(error) {
                        console.log("Error for lcid: "+sticker.lcid + " lsid: "+ sticker.lsid + " in db query", );
                        callback()
                    }
                    else if(sticker) {
                        async.waterfall([
                        ...
                        console.log("Error for lcid: "+sticker.lcid + " lsid: "+ sticker.lsid + " not found in db", );
                        callback()
                    }
                })
            }, (error) => {
