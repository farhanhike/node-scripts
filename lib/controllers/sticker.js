...
exports.updateImageDimension = function() {
	Sticker.find({}, {stickerId:1, lsid:1, lcid:1, images:1}).exec(function(error, stickers) {
		if(error) {
			console.log(error)
		}
		else {
			async.eachLimit(stickers, 20, function(sticker, callback) {
				if (sticker.stickerId) {
					exports.getStickersFromServer(sticker.stickerId, function(error, value) {
						...
						else if(value && value.imageUrls && value.stickerId) {
							var imageUrls;
							var flags = {
								isAnim: false,
...
exports.updateImageDimensionInDb = function(sticker, data, flags, callback) {
	async.parallel({
		...
		if(error) {
			...
			console.log("Updated Image Dimension for stickerId: "+sticker.stickerId + " and animated flag is "+flags.isAnim)
			callback(null, 'done')
		}
	})
}
