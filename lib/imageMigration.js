...
var dbFileName              = "missing.db"
var LegacyDb                = new NeDb({filename: path.join(__dirname, "..", dbFileName), autoload: true})

var mongoose 				= require('mongoose')
    , ObjectId 				= mongoose.Types.ObjectId
    , Sticker   			= mongoose.model('Sticker')
    , LegacySticker         = mongoose.model('LegacySticker')
    , QsReply 				= mongoose.model('QsReply')
	, QsFollow 				= mongoose.model('QsFollow')
...
function updateFixNeDB() {
    var ids = require(path.join(__dirname, '..', 'missing.json'))
    LegacyDb.insert(ids, function(error, vals) {
        if(error) {
            console.log(error)
        }
        else {
...
function getLegacyIdsFromNedb(callback) {
    LegacyDb.find({}).exec(function(error, val) {
        if(error) {
            callback(error, null)
        }
        else {
                ...
                Sticker.findOne({lcid: legacyId.lcid, lsid: legacyId.lsid}, function(error, sticker) {
                    if(error) {
                        ...
                        async.waterfall([
                            ...
                            if(error) {
                                LegacyDb.update({_id: legacyId._id}, {$set: {error: true, errorVal: error.toString()}}, function(dbError, val) {
                        ...
                        LegacyDb.update({_id: legacyId._id}, {$set: {error: true, errorVal: "not found in db"}}, function(dbError, val) {
...
exports.qsReply = function() {
    console.log("qsReply called")
    var json = require('big-json');
    var readStream = fs.createReadStream(path.join(__dirname, "..", 'missing.json'));
    var parseStream = json.createParseStream();
    
    parseStream.on('data', function(val) {
        var id = "007_poof_shocked.png"
        if(val.stkId == id) {
            console.log("Found for stkId : "+ val.stkId)
            return
        }
        var replies = val.StickerInfo || [];
        for(var j=0; j<replies.length; j++) {
            if(replies[j].stkId == id) {
                console.log("Found in : "+ val.stkId)
                return
            }
        }
    });
    ...
    console.log("Does not exists in quick reply")
}
exports.qsReply();

exports.qsFollowUp = function() {
	QsFollow.find({}).skip(0).limit(1000000).exec(function(error, values) {
		if(error) {
			console.log(error)
		}
		else {
			...
			console.log("Does not exists in quick followup")
		}
	})
}